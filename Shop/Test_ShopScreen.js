...
function Test_ShopScreen(props){
  ...
  const panResponder = PanResponder.create({
    ...
    if(totalFirst){
        ...
        pan.setOffset({
            x: 0,
            y: totalFirst === true? 0 : -HEADER_SCROLL_DISTANCE
        });
    }},
    ...
    if(totalFirst){
        ...
        { toValue: { x: 0, y: totalFirst === true ? -HEADER_SCROLL_DISTANCE : 0 },
...
function setInitial(){
  ...
  return(
    ...
    >
      {!isLoading ? <Tab.Navigator backBehavior={'none'} screenOptions={{
          ...
          <Tab.Screen name="임시" children={({navigation})=><Temp/>}/>
  ...
  {(totalFirst)&&<Animated.View
                style={{
                top: HEADER_MAX_HEIGHT+TAB_HEIGHT,
                position: 'absolute',
                transform: [{ translateY: pan.y }],
                width: '100%',
                backgroundColor: 'red',
...
export default Test_ShopScreen;